...
path = 'D:/Dropbox/Current Research/0_Turkish News Classification/TTC-3600_Orj'
...
#categories = ['guncel','ekonomi','kultur-sanat']
news_files = {}
...
if op == 0:
    ...
    # word level tf-idf
    tfidf_vect = TfidfVectorizer(analyzer='word', token_pattern=r'\w{1,}', max_features=MAX_SEQUENCE_LENGTH)
    tfidf_vect.fit(X)
    X_train = tfidf_vect.transform(X_train)
    X_test = tfidf_vect.transform(X_test)
elif op == 2:
    # ngram level tf-idf
    tfidf_vect_ngram = TfidfVectorizer(analyzer='word', token_pattern=r'\w{1,}', ngram_range=(2, 3), max_features=MAX_SEQUENCE_LENGTH)
    ...
    X_train = sequence.pad_sequences(token.texts_to_sequences(X_train), maxlen=MAX_SEQUENCE_LENGTH)
    X_test = sequence.pad_sequences(token.texts_to_sequences(X_test), maxlen=MAX_SEQUENCE_LENGTH)
...
if op == 1 or op == 2 or op == 3 or op == 4:
    ...
    token.fit_on_texts(X)
    word_index = token.word_index
    MAX_NB_WORDS=vocab_size=len(word_index)
